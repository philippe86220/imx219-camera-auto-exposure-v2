...
Exposure is limited to **4–3522** and the IMX219 analogue gain control to **0–232**. The exposure value controls the sensor’s integration time in image lines, rather than directly specifying milliseconds. Analogue gain amplifies the sensor signal, playing a role similar to raising ISO in photography, but its displayed value is a sensor gain setting, not an ISO rating.
